    ...
    public class GenerationController : ControllerBase
    {
        ...
        public async Task<string> Text2Result(string prompt, string artStyle = "realistic")
        {
            using StringContent jsonContent = new(
                ...
                Encoding.UTF8,
                "application/json");
            var result = await _httpClient.PostAsync(string.Format("https://meshy.example.com/v2/text-to-3d"), jsonContent);
        ...
        {
            var result = await _httpClient.GetAsync(string.Format($"https://meshy.example.com/v2/text-to-3d/{resultId}"));
            var responseString = await result.Content.ReadAsStringAsync();
            return responseString;
        }
